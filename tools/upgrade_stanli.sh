...
src_dir = sys.argv[1]
text = open(os.path.join(src_dir, "nuts.cpp")).read()
m = re.search(r'^bool thread_safe_build\(\) \{\n.*?^\}\n', text, re.S | re.M)
assert m, "thread_safe_build() not found -- nuts.cpp changed upstream"
open(os.path.join(src_dir, "thread_safe_build.cpp"), "w").write(
